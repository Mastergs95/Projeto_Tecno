...
namespace BusinessLogicLayer
{
    public class BLL1
    {
        public class Refeicao
        {
            ...
            static public int insertPacks(string nome, string prato, string sobremesa,byte[] foto)
            {
                DAL dal = new DAL();
                SqlParameter[] sqlParams = new SqlParameter[]{
                ...
                return dal.executarNonQuery("INSERT into packs (nome,prato,sobremesa,foto) VALUES (@nome,@prato,@sobremesa,@foto)", sqlParams);
            }
            ...
            static public int insertRefeições(string Nome,string TipodeRefeição, int Calorias, Decimal Lípidos, Decimal Carboidratos, Decimal Proteínas  )
            {
                DAL dal = new DAL();
                SqlParameter[] sqlParams = new SqlParameter[]{
                new SqlParameter("@Nome",Nome),
                ...
                return dal.executarNonQuery("INSERT into Refeiçoes (Nome,TipodeRefeição,Calorias,Lípidos,Carboidratos,Proteínas) VALUES(@Nome,@TipodeRefeição,@Calorias,@Lípidos,@Carboidratos,@Proteínas)", sqlParams);
            }
            static public int updatePacks(int Id, string Nome, string prato, string sobremesa, byte[]foto)
            {
                DAL dal = new DAL();
                SqlParameter[] sqlParams = new SqlParameter[]{
                ...
                return dal.executarNonQuery("update [packs] set [nome]=@nome, [prato]=@prato, [sobremesa]=@sobremesa, [foto]=@foto where [id_pack]=@Id", sqlParams);
            }
            static public int updateRefeições(int ID, string Nome, string TipodeRefeição, int Calorias, Decimal Lípidos, Decimal Carboidratos, Decimal Proteínas)
            {
                DAL dal = new DAL();
                SqlParameter[] sqlParams = new SqlParameter[]{
                new SqlParameter("@ID", ID),
                ...
                return dal.executarNonQuery("update [Refeiçoes] set [Nome]=@Nome, [TipodeRefeição]=@TipodeRefeição, [Calorias]=@Calorias, [Lípidos]=@Lípidos, [Carboidratos]=@Carboidratos, [Proteínas]=@Proteínas where [id_refeicao]=@ID", sqlParams);
            }
